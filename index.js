(function (module, nodeUrl) {
     ...
     * @type {string}
     */
    var decOctect = '(?:' + zeroPad + zeroPad + digitOnly + or + zeroPad + '[1-9]' + digitOnly + or + '1' + digitOnly +
                    digitOnly + or + '2' + '[0-4]' + digitOnly + or + '25' + '[0-5])';
    ...
    IPv6address = '(?:' + IPv6SixHex + or + IPv6FiveHex + or + IPv6FourHex + or + IPv6ThreeeHex + or + IPv6TwoHex + or +
                  IPv6OneHex + or + IPv6NoneHex + or + IPv6NoneHex2 + or + IPv6NoneHex3 + ')';
    ...
    function parseUrl(str) {
        ...
        var matches = str.match(uriRegex);

        if (matches) {
            url.href = matches[0];
            url.protocol = matches[1];
            url.slashes = true;
            url.host = matches[3] && (matches[3] + (matches[4] || '')); // Host & Port (Port is optional)
            url.auth = matches[2];
            url.hostname = matches[3];
            url.port = matches[5];
            url.pathname = matches[6];
            url.search = matches[7];
            url.path = matches[6] && (matches[6] + (matches[7] || '')); // Pathname & Query String (Query String is optional)
            url.query = matches[8];
            url.hash = matches[9];
        }
    ...
    module.exports = {
        parse: parseUrl,
        format: nodeUrl.format, // Defer format and resolve to Node for the time being.
        resolve: nodeUrl.resolve
    };
